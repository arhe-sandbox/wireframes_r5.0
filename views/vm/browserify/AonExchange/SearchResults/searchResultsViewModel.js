(function (app, global) {
    ...
    ns.SearchResultsViewModel = function SearchResultsViewModel() {
        ...
        SearchResultsViewModel.prototype.loadFromJSON = function loadFromJSON(searchResults) {
            var protoSelf = this;
...
(function (app, global) {
    ...
    ns.SearchResultsTabGroupViewModel = function SearchResultsTabGroupViewModel(index) {
        ...
        self.showAllClick = function showAllClick() {
            self.showAll(true);
            app.searchResults.setupSmartHover();
        ...
        SearchResultsTabGroupViewModel.prototype.setAllPlans = function setAllPlans(plans) {
            ...
            for(var i = 0; i < plans.length; i++) {
                plan = plans[i];
                if(!app.plans.AllPlanViewModels[plan.planGuid]) {
                    var planVM = new app.models.PlanViewModel(protoSelf.tabIndex());
                    planVM = planVM.loadFromPlan(plan);
                    newPlans.push(planVM);
                    app.plans.AllPlanViewModels.push(planVM);
                }else {
                    newPlans.push(app.plans.AllPlanViewModels[plan.planGuid]);
                }
            }
