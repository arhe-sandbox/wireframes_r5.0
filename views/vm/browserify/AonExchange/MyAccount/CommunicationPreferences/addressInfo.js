(function (app) {
    //"use strict";
    var ns = app.namespace("EXCHANGE.addressInfo");
    ...
    ns.setupBindings = function setupBindings() {
        ...
        //    $(".about-me-gender").trigger("updateState");
        //    $("#about-me-edit").show();
    ...
    //   ns.bindPrimaryMailingRadios = function () {
    //        $('.primaryAddr').click(function () {
    //            var addrId = $(this).attr('addrId');

    //           // ns.setAsPrimary(addrId);

    //        });
    //        $('.mailingAddr').click(function () {
    //            var addrId = $(this).attr('addrId');

    //            ns.setAsMailing(addrId);

    //        });
    //   };

    //    ns.setAsPrimary = function (addrId, ajaxCallback) {
    //        $.ajax({
    //            type: "POST",
    //            contentType: "application/json; charset=utf-8",
    //            url: "/API/Account/SetAddressAsPrimary",
    //            dataType: "json",
    //            data: JSON.stringify({ 'addressId': addrId }),
    ...
    //    ns.setAsMailing = function (addrId, ajaxCallback) {
    //        $.ajax({
    //            type: "POST",
    //            contentType: "application/json; charset=utf-8",
    //            url: "/API/Account/SetAddressAsMailing",
    //            dataType: "json",
    //            data: JSON.stringify({ 'addressId': addrId }),
    //            response: function (response) {
    //                if ($.isFunction(ajaxCallback)) {
    ...
    //    ns.setFirstAsPrimary = function (addressVm) {
    //        var addressId = addressVm.Id(), newAddressId = addressVm.Id();
    //        var vm = app.viewModels.CommunicationPreferencesViewModel;
    //        for (var i = 0; i < vm.addressVms().length; i++) {
    //            if (vm.addressVms()[i].Id() !== addressId) {
    //                vm.addressVms()[i].isPrimary(true);
    //                vm.addresses()[i].Address.IsPrimary = true;
    //                newAddressId = vm.addressVms()[i].Id();
    //                vm.removePrimaryFromOthers(vm.addressVms()[i]);
    //                break;
    //            }
    //        }

    //        ns.setAsPrimary(newAddressId);
    //    };

    //    ns.setFirstAsMailing = function (addressVm) {
    //        var addressId = addressVm.Id(), newAddressId = addressVm.Id();
    //        var vm = app.viewModels.CommunicationPreferencesViewModel;
    //        for (var i = 0; i < vm.addressVms().length; i++) {
    //            if (vm.addressVms()[i].Id() !== addressId) {
    //                vm.addressVms()[i].isMailing(true);
    //                vm.addresses()[i].Address.IsMailing = true;
    //                newAddressId = vm.addressVms()[i].Id();
    //                vm.removeMailingFromOthers(vm.addressVms()[i]);
    //                break;
    //            }
    //        }

    //        ns.setAsMailing(newAddressId);
    //    };
